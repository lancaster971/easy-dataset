...
class LLMClient {
  ...
  _createClient(provider, config) {
    ...
    // claude-code: proxy lazy-loading (l'SDK è ESM-only, richiede import() dinamico)
    if (providerId === 'claude-code') {
      return {
        _client: null,
        async _getClient() {
          if (!this._client) {
            const ClaudeCodeClient = require('./providers/claude-code.js');
            this._client = new ClaudeCodeClient(config);
          }
          return this._client;
        },
        async chat(m, o) {
          return (await this._getClient()).chat(m, o);
        },
        async chatStream(m, o) {
          return (await this._getClient()).chatStream(m, o);
        },
        async chatStreamAPI(m, o) {
          return (await this._getClient()).chatStreamAPI(m, o);
        }
      };
    }
